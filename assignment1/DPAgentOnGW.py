...
class DPAgentOnGW(AgentOnGW):
    ...
    def pe_step(self):
        ...
        for s in range(self.nS):
            for a in range(self.nA):
                # newQ[s, a] = self.R[s, a] + self.gamma * np.sum(self.T[a][s, :] * curV)
                newQ[s, a] = self.R[s, a] + self.gamma * np.sum(self.T[a][s, :] * self.V)
    ...
    def vi_step(self):   # async backup
        ...
        ##########################################
        # update self.Q using self.V
        for s in range(self.nS):
            for a in range(self.nA):
                self.Q[s, a] = self.R[s, a] + self.gamma * np.sum(self.T[a][s, :] * self.V)
            # self.V[s] = np.max(self.Q[s])  # asynchronous backup
        self.V = np.max(self.Q, axis=1)  # compute V from Q for display purpose
